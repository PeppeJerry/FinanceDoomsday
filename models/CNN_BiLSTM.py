...
class CNNBiLSTM(nn.Module):
    def __init__(self, inputDim, out_target, bi_lstm_layers=2, CNN_out=256, dropout=0.5, company='general',
                 sequence_length=128, SEED=19031999):
        super(CNNBiLSTM, self).__init__()
        random = torch.randint(1, 1000000000, (1,)).item()
        torch.manual_seed(SEED + random)
        ...
        # Fully connected
        self.fc = nn.Linear(2 * CNN_out, inputDim)  # Double because it has forward and backward hidden states
    ...
    def train_model(self, train_loader, x_valid, Y_valid, epochs=200, lr=0.005, lambd=0.05,
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                ...
                if loss_val.item() < loss_val_counter:
                    ...
                    torch.save(self.state_dict(), path + self.company + extra + '.pth')
